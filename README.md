...
By default `npm test` runs all the tests. You can compare `npm run test-callback` with `npm run test-promise` or `npm run test-async-await` to notice performance differences (to list available commands: `npm run`)
